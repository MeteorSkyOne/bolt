...
import (
	...
	cnpb "github.com/meteorsky/kvdb/cmd/proto/coordinator"
	pb "github.com/meteorsky/kvdb/cmd/proto/server"
	"github.com/meteorsky/kvdb/pkg/logger"

	kvdb "github.com/meteorsky/kvdb/internal"
...
func NewDatabaseServer(dbFile, nodeID, cnAddress string) (*DatabaseServer, error) {
	db, err := kvdb.Open(dbFile, 0600, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to open database: %v", err)
	}

	// 创建bucket如果不存在
	err = db.Update(func(tx *kvdb.Tx) error {
...
func (ds *DatabaseServer) restoreDatabase(session *ClientSession, backupData string) string {
	...
	db, err := kvdb.Open(originalPath, 0600, &kvdb.Options{Timeout: 10 * time.Second})
	if err != nil {
...
func (ds *DatabaseServer) validateDatabaseFile(filePath string) error {
	// 尝试打开数据库文件进行验证
	db, err := kvdb.Open(filePath, 0600, &kvdb.Options{
	...
	// 验证数据库结构
	err = db.View(func(tx *kvdb.Tx) error {
		// 检查是否可以正常读取数据库
		return nil
	})
